...
describe('UserRepository authorization versions', () => {
  ...
  it('increments every user assigned to a role including the acting administrator', async () => {
    ...
    await expect(
      repository.incrementAuthorizationVersionsForRole(7, { transaction }),
    ).resolves.toBe(2);
  ...
  it('deduplicates users receiving a changed permission through several roles', async () => {
    const transaction = { id: 'transaction' };
    const findAll = jest
      .spyOn(User, 'findAll')
      .mockResolvedValue([{ id: 2 }, { id: 2 }, { id: 3 }]);
    const increment = jest.spyOn(User, 'increment').mockResolvedValue(undefined);
    const repository = new UserRepository();

    await expect(
      repository.incrementAuthorizationVersionsForPermission(11, { transaction }),
    ).resolves.toBe(2);
